...
namespace spimpl {
    ...
    private:
        static_assert(
            !std::is_array<T>::value, "impl_ptr specialization for arrays is not implemented");
        struct dummy_t_ {
            int dummy__;
        };
    ...
    template <class T, class... Args> inline unique_impl_ptr<T> make_unique_impl(Args&&... args)
    {
...
namespace std {
template <class T, class D, class C> struct hash<jtagpp::spimpl::impl_ptr<T, D, C>> {
    using argument_type = jtagpp::spimpl::impl_ptr<T, D, C>;
