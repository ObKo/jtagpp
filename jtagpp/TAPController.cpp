...
TAPController::TAPController(
    std::shared_ptr<Interface> interface, spimpl::unique_impl_ptr<TAPControllerPrivate>&& p)
...
TAPControllerPtr TAPController::pointer() { return shared_from_this(); }
...
void TAPController::TAPControllerPrivate::walkToState(TAPController::State& current,
    const TAPController::State& target, uint32_t& tms, int& tmsLength)
{
    ...
    if (target == S_IDLE) {
        ...
        walkToState(current, (target == S_CAPTURE_DR) ? S_SELECT_DR_SCAN : S_SELECT_IR_SCAN, tms,
            tmsLength);
        addBits("0", tms, tmsLength);
        current = target;
        ...
        if (current != ((target == S_SHIFT_DR) ? S_EXIT2_DR : S_EXIT2_IR))
            walkToState(
                current, (target == S_SHIFT_DR) ? S_CAPTURE_DR : S_CAPTURE_IR, tms, tmsLength);
        addBits("0", tms, tmsLength);
        current = target;
    } else if ((target == S_EXIT1_DR) || (target == S_EXIT1_IR)) {
        if (current != ((target == S_EXIT1_DR) ? S_SHIFT_DR : S_SHIFT_IR))
            walkToState(
                current, (target == S_EXIT1_DR) ? S_CAPTURE_DR : S_CAPTURE_IR, tms, tmsLength);
        addBits("1", tms, tmsLength);
        current = target;
        ...
        if (current != ((target == S_UPDATE_DR) ? S_EXIT2_DR : S_EXIT2_IR)) {
            if (current == ((target == S_UPDATE_DR) ? S_PAUSE_DR : S_PAUSE_IR))
                walkToState(
                    current, (target == S_UPDATE_DR) ? S_EXIT2_DR : S_EXIT2_IR, tms, tmsLength);
            else
                walkToState(
                    current, (target == S_UPDATE_DR) ? S_EXIT1_DR : S_EXIT1_IR, tms, tmsLength);
        }
        addBits("1", tms, tmsLength);
        current = target;
    }
...
bool TAPController::moveToState(TAPController::State newState)
{
    ...
    Log::debug("TAPController") << "Move from " << d->state << " to " << newState << ": "
                                << dbgString;

    if (current != newState) {
        Log::error("TAPController")
            << "Cannot find path in TAP FSM from " << d->state << " to " << newState;
        return false;
    }
