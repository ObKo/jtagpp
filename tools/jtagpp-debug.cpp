#include <iostream>
#include <fstream>

#include <jtagpp/FTDIInterface.hpp>
#include <jtagpp/TAPController.hpp>
#include <jtagpp/Log.hpp>
#include <jtagpp/XilinxFPGADevice.hpp>
#include <jtagpp/Chain.hpp>

#include <string>
#include <iomanip>

using namespace jtagpp;

const uint8_t bitReverseTable[256] =
{
    ,
};

int main(int argc, char **argv)
{
    Log::instance().addHandler([=](const LogEntry& e) {
        auto& stream = (e.level == LogEntry::ERROR) ? std::cerr : std::cout;
        stream << "[" << e.module << "] " << e.text << std::endl;
    });

    std::string config, bitstream;

    if (argc > 1)
        config = argv[1];

    if (argc > 2)
        bitstream = argv[2];

    InterfacePtr iface = FTDIInterface::create(config);

    iface->setFrequency(10000000);
    iface->open();

    ChainPtr chain = Chain::create(iface);
    std::vector<Device::IDCode> codes = chain->scan();

    if (codes.empty())
        return 0;

    if (bitstream.empty())
        return 0;

    for (const Device::IDCode& code : codes)
        chain->addDevice(XilinxFPGADevice::create(code));

    std::shared_ptr<XilinxFPGADevice> dev =
            std::dynamic_pointer_cast<XilinxFPGADevice>(chain->devices().at(0));

    if (!dev)
        return 0;

    dev->startProgram();

    std::ifstream inf(bitstream);
    uint8_t buf[65536];
    while (!inf.eof())
    {
        inf.read((char*)buf, 65536);
        size_t n = inf.gcount();

        for (int i = 0; i < n; i++)
            buf[i] = bitReverseTable[buf[i]];

        dev->programData(buf, n, inf.eof());
    }

    dev->endProgram();

    return 0;
}
